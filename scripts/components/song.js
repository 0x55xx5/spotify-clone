...
const populateAlbum =async () => {
  await fetch(`https://svelte-demo-mu.vercel.app/api/album/getalbums`, {
    method: 'GET',
    headers: {
...
const songLayout = async(role) => {
  ...
  await fetch(`https://svelte-demo-mu.vercel.app/api/song/getsongbyid?id=${id}`, {
    method: 'GET',
    headers: {
      "apikey": getCookie("session_id")
		}
 ...
 /* getAPI(
    `https://svelte-demo-mu.vercel.app/api/song/getsongbyid?id=${id}`, (data) => {
      const jsonData = JSON.parse(data);
